...
func minD(grid [][]int, moveCost [][]int) int {

	var minnum int = 10 * 1024 * 1024
	...
	for j := 0; j < n; j++ {
		...
		go func(jj int) {
			defer wg.Done()
			recu(grid, moveCost, 0, jj, m, n, 0, 0, &minnum)
		}(j)
	}
	wg.Wait()

	//for j := 0; j < n; j++ {
	//	recu(grid, moveCost, 0, j, m, n, 0, 0, &minnum)
	//}
	return minnum
}

func recu(grid [][]int, moveCost [][]int, i, j, m, n int, numV, numP int, minnum *int) {
	fmt.Printf("i:%d,j:%d\n", i, j)
	...
	if i >= m {
		numV += v
		if numV+numP < *minnum {
			fmt.Printf("min i:%d,j:%d num:%d:%d minnum:%d\n", i, j, numV, numP, *minnum)
			*minnum = numV + numP
		} else {
			fmt.Printf("unmin i:%d,j:%d num:%d:%d minnum:%d\n", i, j, numV, numP, *minnum)
		}
		return
	}

	for jj := 0; jj < n; jj++ {
		recu(grid, moveCost, i, jj, m, n, numV+v, numP+moveCost[v][jj], minnum)
	}
}
